...
const profileTracks = { interview: "workplace", workplace: "workplace", improv: "workplace", speech: "ideas", topic: "ideas" };
const routes = {
  workplace: [
    { code: "CAREER", icon: "◎", name: "求职面试", detail: "结果、判断、证据", profile: "interview", topic: "我需要用一个项目案例说明结果、判断和证据。" },
...
export function mountCandidateHome({ selectProfile, setTopic, showAllProfiles }) {
  ...
  $("#workplaceTrackDescription").textContent = "求职面试、协作推进、汇报故事";
  ...
  function renderRouteGrid() {
    ...
    profileTitle.textContent = selectedTrack === "workplace" ? "职场开口：选择一件要推进的事" : "讲清想法：选择一件要说透的事";
    const grid = $("#modeGrid");
  ...
  function render() {
    ...
    route.textContent = `${config.routes[selectedTrack]} 即兴表达与本地录音可在“查看全部练习”中进入。`;
