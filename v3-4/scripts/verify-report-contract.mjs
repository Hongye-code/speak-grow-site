...
assert(reportFixtures.length === 28, "测试原稿必须为 28 条");
for (const profileId of ["improv", "smalltalk", "workplace", "interview", "story", "topic", "speech"]) assert(reportFixtures.filter((item) => item.profileId === profileId).length === 4, `${profileId} 必须覆盖四条原稿`);
...
console.log("report-contract: 28 fixtures, evidence, detailed sections, BYOK coach, local complete report");
